function webgl_tests() {
        ...
        (function () {
            webgl_dict = webgl();
            for (var i in webgl_dict){
                text += _tmpl(i, webgl_dict[i]);
            }
        })();
        
     
        
        $("#webgl_report_result").html(CryptoJS.SHA1(text).toString());

        $('#append_webgl').append(_tmpl_box('', text, newline=true));
        ...
        if(!webglPixels()){
	        ...
	        setDanger("webgl", true, 15, 0, [["Detected fake webgl", "Some tests shows that you change webgl output"]]);
        }
